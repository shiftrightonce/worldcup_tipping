...
AppDataSource.initialize().then(async () => {
  ...
  const matches = await getMatchesByRound(MatchRound.ROUND_8);

  if (run.tip) {
    await runTippingSimulation(matches, users, MatchRound.ROUND_8);
  }

  if (run.match) {
    await runMatchPlaySimulation(matches, MatchRound.ROUND_8)
  }
